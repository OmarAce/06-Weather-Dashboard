...
var handleForm = function (event) {
  ...
  $("#search-bar")
    .addClass(
      "align-start align-stretch-md align-content-start align-content-stretch-md"
    )
    .removeClass("align-center justify-center");
  $("#search-bar2")
    .addClass("col-md-3 p-3")
    .removeClass("col-md-9 flex-column align-center p-5");
      ...
      fetch(urlOneCall)
        ...
        .then(function (data) {
          ...
          for (let i = 0; i < 1; i++) {
                ...
                <div class="card current mb-3 p-3">
                  <div class="card-body">
                    <h1 class= "border-light border-2 border rounded-pill ml-5 mr-5">${searchTerm} </h3>
                      ...
                      <strong>Wind:</strong> ${data.current.wind_speed.toFixed(
                        1
                      )} mph <br>
            ...
            }" class="img-container border border-dark rounded-circle">
                  </div>
                </div>
              `;
          }
            ...
            forecastContentEl.innerHTML =
              ...
              <div class="col-lg-2 col-md-12 card-background border border-dark mx-auto overflow-hidden m-3 p-3">
                  <div class="card-body text-center">
                    <h3>${dayjs()
                      .add(i + 1, "day")
                      .format("MMM")} </h3> <h4 class="text-muted">${dayjs()
                .add(i + 1, "day")
                .format("MM/DD")} </h4>
                      ...
                      <strong>Wind:</strong> ${data.daily[i].wind_speed.toFixed(
                        1
                      )} mph <br>
...
function getStorage() {
  ...
  if (myStorage) {
    for (var i = 0; i < myStorage.length; i++) searchHistory.push(myStorage[i]);
    initialize();
  }
}
